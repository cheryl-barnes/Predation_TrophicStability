...
## File Structure
Input data (survey and food habits), shapefiles, and model outputs can all be found in Folder 1 ('1_Data' folder). Folders 2 through 5 contain species-specific analyses for each component of the predation index (component estimates are stored in the data folder). Folder 6 contains overall predation indices (all predators combined) for each of the spatial scales of interest (<i>i.e.</i>, basin, area encompassed by the stock assessment for Gulf of Alaska pollock, subregion, and statistical area). Script files in folder 6 also include variance ratio calculations, which enabled estimates of synchrony and portfolio effects. Specific analyses that resulted in publication tables and figures are noted throughout. 
...
The authors would like to acknowledge Terry Quinn for offering his insight and expertise during various stages of this project. We have dedicated this work to him.
