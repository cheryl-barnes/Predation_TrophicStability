# This script file includes code necessary to calculate fork length- and biomass-weighted proportions of pollock (Chipps and Garvey 2007) consumed by Walleye Pollock. The analyses below rely on standardized bottom trawl surveys (Gulf of Alaska, 1990 to 2015) conducted by the Resource Assessment and Conservation Engineering (RACE) Division of the Alaska Fisheries Science Center (AFSC, National Marine Fisheries Service, National Oceanic and Atmospheric Association [NOAA]). Food habits data (Gulf of Alaska, 1990 to 2015) were provided by the Resource Ecology and Ecosystem Modeling (REEM) Program and are publicly accessible at https://access.afsc.noaa.gov/REEM/WebDietData/DietDataIntro.php. See von Szalay and Raring (2016) and Livingston et al. (2017) for data collection methods.
...
preyData = read.csv("1_Data/GOA_RawPP_WEPdiets.csv") 
...
preyWTcontents$FLBin = cut(preyWTcontents$Pred_len, breaks = c(0,16,26,36,46,56,66,76,86,99))
levels(preyWTcontents$FLBin) = c("<=16", "17-26", "27-36", "37-46", "47-56", "57-66", "67-76", "77-86", ">=97")
...
# Create 10-cm predator length bins:
lengths$FLBin = cut(lengths$PredLength, breaks = c(0,16,26,36,46,56,66,76,86,99))
levels(lengths$FLBin) = c("<=16", "17-26", "27-36", "37-46", "47-56", "57-66", "67-76", "77-86", ">=97")
...
colnames(preyWTgrid[16]); colnames(preyWTgrid[86]); ncol(preyWTgrid)
preyWTlong = melt(preyWTgrid, id.vars = c("id2", "EEZgrid", "Year", "INPFC_Area", "Haul_Join", "Rlong", "Rlat", "Pred_name", "PredLength", "FLBin"), measure.vars = 16:86, variable.name = "Prey_Name", value.name = "preyWT_g")
preyWTlong = na.omit(preyWTlong)
preyWTlong$Pred_name = "Walleye Pollock"
...
preyWTweighted37 = subset(preyWTweighted, PredLength >= 37)
...
WEPabunPred = readRDS("2_RelativePredatorDensities/normWEPabun.rds")
WEPabunPred = rename(WEPabunPred, Year = YEAR) # match other index components
WEPabunPred$Year = as.numeric(as.character(WEPabunPred$Year))

BioWT = WEPabunPred %>%
  group_by(Year) %>% 
  mutate(BioWT = WEPpredAbun / mean(WEPpredAbun)) 
BioWT = as.data.frame(BioWT)
BioWT$Year = as.numeric(BioWT$Year)
...
# Select only sizes of fish encompassed in total predator biomass estimates:
WeightedPreyData_37 = subset(WeightedPreyData, PredLength >= 37)
WeightedPreyData_37 = na.omit(WeightedPreyData_37)

# Save results:
saveRDS(WeightedPreyData_37, "4_ProportionsPollockConsumed/propWEP_WEP_37.rds")

# Plot proportions of pollock consumed, by survey year:
preyWT_YR = ggplot(WeightedPreyData_37, aes(x = Year, y = WTdiet, fill = Prey_Name, order = -as.numeric(Prey_Name))) + 
