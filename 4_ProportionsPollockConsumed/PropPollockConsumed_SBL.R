# This script file includes code necessary to calculate fork length- and biomass-weighted proportions of pollock (Chipps and Garvey 2007) consumed by Sablefish. The analyses below rely on standardized bottom trawl surveys (Gulf of Alaska, 1990 to 2015) conducted by the Resource Assessment and Conservation Engineering (RACE) Division of the Alaska Fisheries Science Center (AFSC, National Marine Fisheries Service, National Oceanic and Atmospheric Association [NOAA]). Food habits data (Gulf of Alaska, 1990 to 2015) were provided by the Resource Ecology and Ecosystem Modeling (REEM) Program and are publicly accessible at https://access.afsc.noaa.gov/REEM/WebDietData/DietDataIntro.php. See von Szalay and Raring (2016) and Livingston et al. (2017) for data collection methods.
...
preyData = read.csv("1_Data/GOA_RawPP_SBLdiets.csv") 
...
preyWTcontents$FLBin = cut(preyWTcontents$Pred_len, breaks = c(0,14,24,34,44,54,64,74,84,94,110))
levels(preyWTcontents$FLBin) = c("<=14", "15-24", "25-34", "35-44", "45-54", "55-64", "65-74", "75-84", "85-94", ">95")
preyWTcontents = rename(preyWTcontents, PredLength = Pred_len)

##########################################################
# Read in and format length data from bottom trawls:
lengths = read.csv("1_Data/race_length_by_haul_SBL.csv", header=T, skip=7)
...
# Create 10-cm predator length bins:
lengths$FLBin = cut(lengths$PredLength, breaks = c(0,14,24,34,44,54,64,74,84,94,110))
...
# Find where length data overlap with clipped grid and convert back to a data frame:
...
# Join length and geographic data:
...
colnames(preyWTgrid[16]); colnames(preyWTgrid[84]); ncol(preyWTgrid)
preyWTlong = melt(preyWTgrid, id.vars = c("id2", "EEZgrid", "Year", "INPFC_Area", "Haul_Join", "Rlong", "Rlat", "Pred_name", "PredLength", "FLBin"), measure.vars = 16:84, variable.name = "Prey_Name", value.name = "preyWT_g")
...
levels(preyWTlong$Prey_Name) = list(Scorpaeniformes="Agonidae", Trachiniformes="Ammodytidae", Pleuronectiformes="Arrowtooth.flounder", Scorpaeniformes="Atka.Mackeral", Argentiniformes="Bathylagid", Mollusca="Bivalvia", Echinodermata="Brittle.Star", Crustacea="Cancridea", Misc.Shrimps="Capreillidea", Other="Chaetognatha", Misc.Crabs="Chionoecetes.spp.", Clupeiformes="Clupeoidei", Cnidaria="Cnidaria", Crustacea="Copepoda", Scorpaeniformes="Cottid", Misc.Shrimps="Crangonidae..shrimp.", Ctenophora="Ctenophora", Crustacea="Cumacea", Scorpaeniformes="Cyclopteridae", Euphausiacea="Euphausiacea", Teleostei="Fish.Eggs", Pleuronectiformes="Flathead.sole", Crustacea="Gammaridea", Mollusca="Gastropod", Pleuronectiformes="Greenland.turbot", Misc.Shrimps="Hippolytidae..shrimp.", Misc.Shrimps="Hyperiidea", Crustacea="Isopoda", Tunicata="Larvacea", Gadiformes="Macrouridae", Other="Misc", Crustacea="Misc.Amphipoda", Misc.Crabs="Misc.Anomura", Misc.Crabs="Misc.Brachyura", Mollusca="Misc.Cephalopoda", Misc.Crabs="Misc.Crab", Crustacea="Misc.Crustacea", Crustacea="Misc.Decapoda", Echinodermata="Misc.Echinoderm", Pleuronectiformes="Misc.Flatfish", Gadiformes="Misc.Gadidae", Other="Misc.Invert", Misc.Crabs="Misc.Lithodidae", Misc.Crabs="Misc.Majidae", Mollusca="Misc.Mollusca", Chondrichthyes="Misc.Non.teleost.fish", Other="Misc.Org", Misc.Shrimps="Misc.Shrimp", Teleostei="Misc.Teleost", Annelida="Misc.Worm", Myctophiformes="Myctophidae", Misc.Shrimps="Mysidacea", Octopoda="Octopoda", Offal="Offal", Osmeriformes="Osmerid", Gadus.macrocephalus="Pacific.Cod", Pleuronectiformes="Pacific.halibut", Trachiniformes="Pacific.sandfish", Misc.Crabs="Paguridae", Misc.Shrimps="Pandalidae..shrimp.", Annelida="Polychaeta", Mollusca="Pteropoda", Chondrichthyes="Rajadae", Salmoniformes="Salmonidae", Echinodermata="Sand.Dollar", Echinodermata="Sea.Urchin", Scorpaeniformes="Sebastelobus", Scorpaeniformes="Sebastes", Pleuronectiformes="Southern.rock.sole", Perciformes="Stichaeidae", Misc.Crabs="Tanner.Crab", Teuthida="Teuthida", Tunicata="Tunicate", Other="Unid.Eggs", Scorpaeniformes="Unid.Rockfish", Gadus.chalcogrammus="Walleye.pollock", Perciformes="Zoarcoidae")
...
# Insert rows for years with no food habits data:
preyWTlong45 = subset(preyWTlong, PredLength >= 45)
preyWTlong45 = add_row(preyWTlong45)
preyWTlong45[nrow(preyWTlong45), ] = c(NA, NA, 2013, NA, NA, NA, NA, NA, NA, NA, NA, 0)
preyWTlong45 = add_row(preyWTlong45)
preyWTlong45[nrow(preyWTlong45), ] = c(NA, NA, 2015, NA, NA, NA, NA, NA, NA, NA, NA, 0)

##########################################################
### CALCULATE AND PLOT WEIGHTED PROPORTIONS OF PREY BY MASS ###
##########################################################
...
# Calculate sample weights from dividing length-based proportions of fish caught by the bottom trawl survey by those subsampled for gut content analysis in each survey year and grid cell:
...
preyWTweighted45 = subset(preyWTweighted, PredLength >= 45)

# Add entries for years with no food habits data:
preyWTweighted45 = add_row(preyWTweighted45)
preyWTweighted45[nrow(preyWTweighted45), ] = c(NA, NA, 2013, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 0)
preyWTweighted45 = add_row(preyWTweighted45)
preyWTweighted45[nrow(preyWTweighted45), ] = c(NA, NA, 2015, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 0)
...
SBLabunPred = readRDS("2_RelativePredatorDensities/normSBLabun.rds")
SBLabunPred = rename(SBLabunPred, Year = YEAR) # match other index 
SBLabunPred$Year = as.numeric(as.character(SBLabunPred$Year))

BioWT = SBLabunPred %>%
  group_by(Year) %>% 
  mutate(BioWT = SBLcpue_fit / mean(SBLcpue_fit )) 
BioWT = as.data.frame(BioWT)
BioWT$Year = as.numeric(as.character(BioWT$Year))
...
# Select only sizes of fish encompassed in total predator biomass estimates:
WeightedPreyData_45 = subset(WeightedPreyData, PredLength >= 45)
WeightedPreyData_45 = na.omit(WeightedPreyData_45)

# Save results:
saveRDS(WeightedPreyData_45, "4_ProportionsPollockConsumed/propWEP_SBL_45.rds")

# Plot proportions of pollock consumed, by survey year:
preyWT_YR = ggplot(WeightedPreyData_45, aes(x = Year, y = WTdiet, fill = Prey_Name, order = -as.numeric(Prey_Name))) + 
