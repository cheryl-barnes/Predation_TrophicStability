# This script file includes code necessary to calculate fork length- and biomass-weighted proportions of pollock (Chipps and Garvey 2007) consumed by Pacific Halibut. The analyses below rely on standardized bottom trawl surveys (Gulf of Alaska, 1990 to 2015) conducted by the Resource Assessment and Conservation Engineering (RACE) Division of the Alaska Fisheries Science Center (AFSC, National Marine Fisheries Service, National Oceanic and Atmospheric Association [NOAA]). Food habits data (Gulf of Alaska, 1990 to 2015) were provided by the Resource Ecology and Ecosystem Modeling (REEM) Program and are publicly accessible at https://access.afsc.noaa.gov/REEM/WebDietData/DietDataIntro.php. See von Szalay and Raring (2016) and Livingston et al. (2017) for data collection methods.
...
preyData = read.csv("1_Data/GOA_RawPP_PHdiets.csv") 
...
# Create 20-cm predator length bins:
preyWTcontents$FLBin = cut(preyWTcontents$Pred_len, breaks = c(0,21,41,61,81,101,121,141,161,181,201,211))
levels(preyWTcontents$FLBin) = c("<=21", "22-41", "42-61", "62-81", "82-101", "102-121", "122-141", "142-161", "162-181", "182-201", ">=202")
preyWTcontents = rename(preyWTcontents, PredLength = Pred_len)

##########################################################
# Read in and format length data from bottom trawls:
lengths = read.csv("1_Data/race_length_by_haul_PH.csv", header=T, skip=7)
...
# Create 20-cm predator length bins:
lengths$FLBin = cut(lengths$PredLength, breaks = c(0,21,41,61,81,101,121,141,161,181,201,211))
levels(lengths$FLBin) = c("<=21", "22-41", "42-61", "62-81", "82-101", "102-121", "122-141", "142-161", "162-181", "182-201", ">=202")
...
colnames(preyWTgrid[16]); colnames(preyWTgrid[96]); ncol(preyWTgrid)
preyWTlong = melt(preyWTgrid, id.vars = c("id2", "EEZgrid", "Year", "INPFC_Area", "Haul_Join", "Rlong", "Rlat", "Pred_name", "PredLength", "FLBin"), measure.vars = 16:96, variable.name = "Prey_Name", value.name = "preyWT_g")
preyWTlong = na.omit(preyWTlong)
preyWTlong$Pred_name = "Pacific Halibut"
...
preyWTweighted82 = subset(preyWTweighted, PredLength >= 82)
...
PHabunPred = readRDS("2_RelativePredatorDensities/normPHabun.rds") # trawl survey (significant correlation with setline survey)

BioWT = PHabunPred %>%
  group_by(Year) %>% 
  mutate(BioWT = PHpredAbun / mean(PHpredAbun)) 
...
# Assign mean biomass weight to years without predictions (1990, 1993, 1996):
WeightedPreyData = WeightedPreyData %>%
  group_by(id2) %>%
  mutate(meanBioWT = mean(na.omit(BioWT)))

WeightedPreyData$BioWT = WeightedPreyData$BioWT
WeightedPreyData$BioWT[is.na(WeightedPreyData$BioWT)] = WeightedPreyData$meanBioWT[is.na(WeightedPreyData$BioWT)]
WeightedPreyData$WTdiet = WeightedPreyData$preyFLwt * WeightedPreyData$BioWT

# Select only sizes of fish encompassed in total predator biomass estimates:
WeightedPreyData_82 = subset(WeightedPreyData, PredLength >= 82)
WeightedPreyData_82 = subset(WeightedPreyData, WTdiet != "NaN")

# Save results:
saveRDS(WeightedPreyData_82, "4_ProportionsPollockConsumed/propWEP_PH_82.rds")

# Plot proportions of pollock consumed, by survey year:
preyWT_YR = ggplot(WeightedPreyData_82, aes(x = Year, y = WTdiet, fill = Prey_Name, order = -as.numeric(Prey_Name))) + 
