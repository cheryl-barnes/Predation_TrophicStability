# This script file includes code necessary to calculate subregional indices of predation for Walleye Pollock in the Gulf of Alaska (MT per year; 1990 to 2015). Pollock predator: Walleye Pollock (Gadus chalcogrammus) conspecifics. 
...
# Assign GOA subregions based on grid cell id:
Ration_WEP$SubRegion = with(Ration_WEP, 
ifelse(id2 %in% c(123,150,151,152,153,154,155,156,178,179,180,181,182,183,184,185,186,187,188,189,190,206,20,208,209,210,211,212,213,214,215,236,237,238,239,240,241,242,268,269,270,271,272,299), "Western", 
ifelse(id2 %in% c(216,217,218,243,244,245,246,247,273,274,275,276,277,278,279,300,301,302,303,304,305,306,333,334,335,336,337,338,369,370,371,372,403,404,405,406,437,438,439,470), "Central", 
ifelse(id2 %in% c(280,307,308,339,340,341,373,374,375,376,407,408,409,410,411,440,441,442,443,444,445,446,471,472,473,474,475,476,477,499,500,501,502,503,504,505,525,526,527,528,529,530,531,532,533,551,552,553,554,555,556,557,558,578,579,580,582), "Central", 
ifelse(id2 %in% c(534,540,541,559,560,561,562,563,564,565,566,583,584,585,586,587,588,589,590,604,605,608,609), "Eastern", 
...
# Eliminate locations outside of the study area (western, central, and eastern GOA):
Ration_WEP$SubRegion = as.factor(Ration_WEP$SubRegion)
Ration_WEP = subset(Ration_WEP, SubRegion != "Other")
Ration_WEP$SubRegion = ordered(as.factor(Ration_WEP$SubRegion), levels=c("Western", "Central", "Eastern"))
...
# Assign GOA subregions based on grid cell id:
propWEP_WEP$SubRegion = with(propWEP_WEP, 
ifelse(id2 %in% c(123,150,151,152,153,154,155,156,178,179,180,181,182,183,184,185,186,187,188,189,190,206,20,208,209,210,211,212,213,214,215,236,237,238,239,240,241,242,268,269,270,271,272,299), "Western", 
ifelse(id2 %in% c(216,217,218,243,244,245,246,247,273,274,275,276,277,278,279,300,301,302,303,304,305,306,333,334,335,336,337,338,369,370,371,372,403,404,405,406,437,438,439,470), "Central", 
ifelse(id2 %in% c(280,307,308,339,340,341,373,374,375,376,407,408,409,410,411,440,441,442,443,444,445,446,471,472,473,474,475,476,477,499,500,501,502,503,504,505,525,526,527,528,529,530,531,532,533,551,552,553,554,555,556,557,558,578,579,580,582), "Central", 
ifelse(id2 %in% c(534,540,541,559,560,561,562,563,564,565,566,583,584,585,586,587,588,589,590,604,605,608,609), "Eastern", 
...
# Eliminate locations outside of the study area (western, central, and eastern GOA):
propWEP_WEP$SubRegion = as.factor(propWEP_WEP$SubRegion)
propWEP_WEP = subset(propWEP_WEP, SubRegion != "Other")
propWEP_WEP$SubRegion = ordered(as.factor(propWEP_WEP$SubRegion), levels=c("Western", "Central", "Eastern"))
...
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(1999, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2001, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2003, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2005, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2007, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2009, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2011, "Western", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2005, "Central", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2011, "Central", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2015, "Central", "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(1993, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2003, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2005, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2007, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2009, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2011, "Eastern",  "Gadus.chalcogrammus", 0, 0)
propWEP_WEPsub = add_row(as.data.frame(propWEP_WEPsub))
propWEP_WEPsub[nrow(propWEP_WEPsub), ] = c(2015, "Eastern",  "Gadus.chalcogrammus", 0, 0)
...
WEPpredWEP = subset(WEPpredWEP, SubRegion != "Eastern" | Year < 1996 | Year > 2001)

#########################################################
# Calculate year-specific predation by Arrowtooth Flounder (g; all pollock age classes combined):
WEPpredWEP$propWT = as.numeric(WEPpredWEP$propWT)
...
WEPpredWEP_age_t_max = melt(WEPpredWEP_age, id.vars = c("Year", "SubRegion"), measure.vars = c("predWEP_t_max","predWEP_t_max_Age0","predWEP_t_max_Age1"), variable.name = "AgeClass", value.name = "MT")
