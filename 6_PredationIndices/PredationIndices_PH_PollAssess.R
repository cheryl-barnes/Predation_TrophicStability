# This script file includes code necessary to calculate an index of predation for Walleye Pollock within the assessment are of the Gulf of Alaska (MT per year; 1990 to 2015). Pollock predator: Pacific Halibut (Hippoglossus stenolepis). 
...
# Assign mean relative predator densities to years with missing setline survey data (i.e., 1990, 1993, and 1996):
meanRPD = mean(RPD_sum$NormAbun)
RPD_sum = add_row(RPD_sum)
RPD_sum[nrow(RPD_sum), 1:2] = c(1990, meanRPD)
RPD_sum = add_row(RPD_sum)
RPD_sum[nrow(RPD_sum), 1:2] = c(1993, meanRPD)
RPD_sum = add_row(RPD_sum)
RPD_sum[nrow(RPD_sum), 1:2] = c(1996, meanRPD)

# Remove years not sampled by the bottom trawl survey (for comparability with other pollock predators):
