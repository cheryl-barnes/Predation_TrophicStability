# This script file includes code necessary to calculate mean annual rations for Sablefish (g per g per yr). Bioenergetics model parameters used to estimate maximum daily consumption (Cmax; g per g per d), the temperature scaling function, and mean relative foraging rates (RFR) were obtained from Harvey (2009) and Armstrong and Schindler (2011). Mean region- and size-specific number of foraging days per year were specified in Harvey (2009). 
...
##########################################################
# Read in and format food habits data for Sablefish:
FHdata = read.csv("1_Data/GOA_RawPP_SBLdiets.csv") 
...
# Select predator sizes of interest (SBL >= 45 cm):
FHdata = subset(FHdata, Pred_len >= 45)
...
INPFC_Pr = spTransform(INPFC, CRS("+proj=utm +zone=5, +datum=WGS84 +units=km +no_defs")) 
  # UTM projection maintains cnsistent grid cell area regardless of geographic location.
...
FHdata_grid$EFD = 365 # assumed (no reference)
...
saveRDS(FHdata_grid, "3_MeanAnnualRations/Rations_SBL.rds")

##########################################################
### MEAN ANNUAL RATIONS ###
##########################################################
# Calculate mean and sd Cmax (g/g/y) for each survey year:
Cmax_ggy_SBL_mean = FHdata_grid %>%
  group_by(Year) %>%
  summarise(meanCmax_ggy = mean(Cmax_ggy)) 
Cmax_ggy_SBL_sd = FHdata_grid %>%
  group_by(Year) %>%
  summarise(sdCmax_ggy = sd(Cmax_ggy)) 
Cmax_ggy_SBL = merge(Cmax_ggy_SBL_mean, Cmax_ggy_SBL_sd)

# Calculate 95% confidence intervals:
Cmax_ggy_SBL$CI_Hi = Cmax_ggy_SBL$meanCmax_ggy + (1.96*Cmax_ggy_SBL$sdCmax_ggy)
Cmax_ggy_SBL$CI_Lo = Cmax_ggy_SBL$meanCmax_ggy - (1.96*Cmax_ggy_SBL$sdCmax_ggy)

# No diet data collected in 2013 or 2015. Assume overall mean rations:
Cmax_ggy_SBL[11,] = c(2013, mean(FHdata_grid$Cmax_ggy), NA, NA, NA)
Cmax_ggy_SBL[12,] = c(2015, mean(FHdata_grid$Cmax_ggy), NA, NA, NA)

# Plot results (part of Fig. 2):
MeanAnnualRation_SBL = ggplot(Cmax_ggy_SBL, aes(x = Year, y = meanCmax_ggy)) +
  geom_line(col="orange1", lwd=1) +
  geom_point(size = 3.5, col="darkorange2", stroke=2, shape=3, alpha = 1) +
