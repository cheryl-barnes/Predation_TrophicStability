# This script file includes code necessary to calculate mean annual rations for Pacific Cod (g per g per yr). Bioenergetics model parameters used to estimate maximum daily consumption (Cmax; g per g per d), the temperature scaling function, and mean relative foraging rates (RFR) were obtained from Holsman and Aydin (2015) and Holsman et al. (in prep). Mean region- and size-specific number of foraging days per year were specified in Holsman and Aydin (2015). 
...
##########################################################
# Read in and format food habits data for Pacific Cod:
FHdata = read.csv("1_Data/GOA_RawPP_PCdiets.csv") 
...
# Select predator sizes of interest (PC >= 0 cm):
FHdata = subset(FHdata, Pred_len >= 0)
...
# Multiply Cmax by effective number of foraging days (EFD; Holsman and Aydin 2015) to scale to year:
  # EFD PC <  55 cm = 365 d
  # EFD PC >= 55 cm = 329 d
FHdata_grid$EFD = ifelse(FHdata_grid$Pred_len < 55, 365, 329)
FHdata_grid$Cmax_ggy = FHdata_grid$Cmax_ggd * FHdata_grid$EFD
FHdata_grid = subset(FHdata_grid, Cmax_ggy !="Inf")

# Remove INPFC statistical areas 649 and 659 (inside waters of Southeast Alaska) and 0 and 519 (outside study area):
FHdata_grid = subset(FHdata_grid, INPFC_Area != "0" & INPFC_Area != "519" & INPFC_Area != "649" & INPFC_Area != "659")
saveRDS(FHdata_grid, "3_MeanAnnualRations/Rations_PC.rds")

##########################################################
### MEAN ANNUAL RATIONS ###
##########################################################
# Calculate mean and sd Cmax (g/g/y) for each survey year:
Cmax_ggy_PC_mean = FHdata_grid %>%
  group_by(Year) %>%
  summarise(meanCmax_ggy = mean(Cmax_ggy)) 
Cmax_ggy_PC_sd = FHdata_grid %>%
  group_by(Year) %>%
  summarise(sdCmax_ggy = sd(Cmax_ggy)) 
Cmax_ggy_PC = merge(Cmax_ggy_PC_mean, Cmax_ggy_PC_sd)

# Calculate 95% confidence intervals:
Cmax_ggy_PC$CI_Hi = Cmax_ggy_PC$meanCmax_ggy + (1.96*Cmax_ggy_PC$sdCmax_ggy)
Cmax_ggy_PC$CI_Lo = Cmax_ggy_PC$meanCmax_ggy - (1.96*Cmax_ggy_PC$sdCmax_ggy)

# Plot results (part of Fig. 2):
MeanAnnualRation_PC = ggplot(Cmax_ggy_PC, aes(x = Year, y = meanCmax_ggy)) +
  geom_line(col="chartreuse2", lwd=1) +
  geom_point(size = 3.5, col="chartreuse4", stroke=2, shape=3, alpha = 1) +
