...
##########################################################
# Read in and format food habits data for Walleye Pollock:
FHdata = read.csv("1_Data/GOA_RawPP_WEPdiets.csv") 
...
# Select predator sizes of interest (WEP >= 37 cm):
FHdata = subset(FHdata, Pred_len >= 37)
...
# Remove INPFC statistical areas 0 and 659 (unidentifed or inside waters of Southeast Alaska):
FHdata_grid = subset(FHdata_grid, INPFC_Area != "0" & INPFC_Area != "659")
saveRDS(FHdata_grid, "3_MeanAnnualRations/Rations_WEP.rds")

##########################################################
### MEAN ANNUAL RATIONS ###
##########################################################
# Calculate mean and sd Cmax (g/g/y) for each survey year:
Cmax_ggy_WEP_mean = FHdata_grid %>%
  group_by(Year) %>%
  summarise(meanCmax_ggy = mean(Cmax_ggy)) 
Cmax_ggy_WEP_sd = FHdata_grid %>%
  group_by(Year) %>%
  summarise(sdCmax_ggy = sd(Cmax_ggy)) 
Cmax_ggy_WEP = merge(Cmax_ggy_WEP_mean, Cmax_ggy_WEP_sd)

# Calculate 95% confidence intervals:
Cmax_ggy_WEP$CI_Hi = Cmax_ggy_WEP$meanCmax_ggy + (1.96*Cmax_ggy_WEP$sdCmax_ggy)
Cmax_ggy_WEP$CI_Lo = Cmax_ggy_WEP$meanCmax_ggy - (1.96*Cmax_ggy_WEP$sdCmax_ggy)

# Plot results (part of Fig. 2):
MeanAnnualRation_WEP = ggplot(Cmax_ggy_WEP, aes(x = Year, y = meanCmax_ggy)) +
  geom_line(col="lightskyblue1", lwd=1) +
  geom_point(size = 3.5, col="deepskyblue1", stroke=2, shape=3, alpha = 1) +
