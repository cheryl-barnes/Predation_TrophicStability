# This script file includes code necessary to calculate mean annual rations for Pacific Halibut (g per g per yr). Bioenergetics model parameters used to estimate maximum daily consumption (Cmax; g per g per d), the temperature scaling function, and mean relative foraging rates (RFR) were obtained from Holsman et al. (2019). Mean region- and size-specific number of foraging days per year were also specified in Holsman and Aydin (2015). 
...
##########################################################
# Read in and format food habits data for Pacific Halibut:
FHdata = read.csv("1_Data/GOA_RawPP_PHdiets.csv") 
...
# Select predator sizes of interest (PH >= 82 cm)::
FHdata = subset(FHdata, Pred_len >= 82)
...
##########################################################
# Compute temperature scaling function for each observation (parameter estimates from Holsman et al. 2019):
...
# Remove statistical area 659 (inside waters of Southeast Alaska):
FHdata_grid = subset(FHdata_grid, INPFC_Area != "659")
saveRDS(FHdata_grid, "3_MeanAnnualRations/Rations_PH.rds")

##########################################################
### MEAN ANNUAL RATIONS ###
##########################################################
# Calculate mean and sd Cmax (g/g/y) for each survey year:
Cmax_ggy_PH_mean = FHdata_grid %>%
  group_by(Year) %>%
  summarise(meanCmax_ggy = mean(Cmax_ggy)) 
Cmax_ggy_PH_sd = FHdata_grid %>%
  group_by(Year) %>%
  summarise(sdCmax_ggy = sd(Cmax_ggy)) 
Cmax_ggy_PH = merge(Cmax_ggy_PH_mean, Cmax_ggy_PH_sd)

# Calculate 95% confidence intervals:
Cmax_ggy_PH$CI_Hi = Cmax_ggy_PH$meanCmax_ggy + (1.96*Cmax_ggy_PH$sdCmax_ggy)
Cmax_ggy_PH$CI_Lo = Cmax_ggy_PH$meanCmax_ggy - (1.96*Cmax_ggy_PH$sdCmax_ggy)

# Plot results (part of Fig. 2):
MeanAnnualRation_PH = ggplot(Cmax_ggy_PH, aes(x = Year, y = meanCmax_ggy)) +
  geom_line(col="indianred2", lwd=1) +
  geom_point(size = 3.5, col="red", stroke=2, shape=3, alpha = 1) +
